  ...
  return (
    ...
    >
        ...
        <div className="mr-3">
            ...
            <input type="file" ref={ inputFileRef } hidden accept={ accept } onChange={ ( e ) => setSelectedFile(e.target.files?.[0]) } />
        </div>
        ...
        <div className="ml-4">
            <button className="btn-primary" disabled={ !selectedFile }>
                {
                    !selectedFile ? <span className="mr-2">Enviar</span> : <span className="mr-2">{ selectedFile.name.substring(0, 15) } { selectedFile.name.length > 15 && '...' }</span>
                }
